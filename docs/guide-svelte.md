...
`getClient()` must be called during component initialization (top level of the `<script>` block), and only inside a `<Cartographer>` provider. It returns the same `CartographerClient` instance documented in the [Client SDK](guide-app-server.md#client-sdk) section of the Application Server guide.
