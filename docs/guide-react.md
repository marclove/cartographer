...
`useClient()` must be called inside a `<CartographerProvider>`. It returns the same `CartographerClient` instance documented in the [Client SDK](guide-actor-framework.md#client-sdk) section of the Actor Framework guide.
