# Task 22: Multi-Step Action Workflow Integration Test

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Test suspension and resumption via `actionReceived` / `untilSuccess` in a realistic document review pipeline.

**Architecture:** Single integration test file with one test. Tree: analyze → emit findings → wait for decision → branch on approve/reject. Exercises sequence resumption, action consumption, emitToClient, and mapPayload across multiple messages.

**Tech Stack:** TypeScript, vitest

**Key files to understand:**
- `src/__integration__/helpers.ts` — `setupTest` harness
- `src/application/action-received.ts` — `actionReceived(name, { mapPayload })` factory
- `src/application/emit-to-client.ts` — `emitToClient(name, dataFn)` factory
- `src/application/until-success.ts` — `untilSuccess(child)` factory
- `docs/superpowers/specs/2026-03-16-full-stack-feature-tests-design.md` — test 1 spec

---

### Step 1: Create the test file

Create `src/__integration__/multi-step-action-workflow.test.ts`:
      ...
      createTree: () =>
        new BehaviorTree({
          ...
          root: new SequenceNode({
            name: 'main',
            children: [
              // Step 1: Agent analyzes the document
              new ActionNode({
                name: 'analyze',
                action: (ctx) => {
                  ctx.blackboard.set('analysis', {
              ...
              // Step 2: Emit findings to the client
              emitToClient('ui:findings', (ctx) => ctx.blackboard.get('analysis')),

              // Step 3: Wait for user decision (approve or reject)
              untilSuccess(
                new SelectorNode({
                  name: 'wait-decision',
                  children: [
...
Run: `npm run test:integration -- multi-step-action-workflow`
Expected: PASS
...
```bash
git add src/__integration__/multi-step-action-workflow.test.ts
git commit -m "test: add multi-step action workflow integration test"
```
