...
**Architecture:** A `InMemoryBlackboard` backed by a `Map<string, unknown>`. Scoped blackboards prefix all keys with `namespace:` and delegate to the parent map.
...
Expected: FAIL — cannot import `InMemoryBlackboard`

### Step 3: Implement InMemoryBlackboard
